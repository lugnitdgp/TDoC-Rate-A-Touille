...
public class UserDbHandler {
    ...
    //register
    public static void registerStaff(String name,String password,int age,double salary){
        try(
       ...
       )
        {
            ...
            String encryptedPassword=Utils.encryptPassword(password,salt);
            String sqlinsert="INSERT INTO staff(name,salt,password,age,salary) VALUES('"+name+"','"+salt+"','"+encryptedPassword+"',"+age+","+salary+")";
            int count=stmt.executeUpdate(sqlinsert);
            if(count!=0) System.out.println("staff successfully registered");         
        }
    ...
    public boolean loginStaff(String name,String password){
        ...
        Statement stmt = conn.createStatement();
            ...
            //code TBD
                 ...
                 String givenPass=Utils.encryptPassword(password, dbSalt);
           ...
           if(err.equals("Illegal operation on empty result set.")){
               System.out.println("No such user exists");
           }else
                ex.printStackTrace();
