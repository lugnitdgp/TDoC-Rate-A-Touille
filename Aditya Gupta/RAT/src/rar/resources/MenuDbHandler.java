...
public class MenuDbHandler {
    ...
    public void AddToMenu(String name, double price, int qty, int tpp, int nppt){
        try(
            Connection conn = DriverManager.getConnection(StringConstants.DB_URL,StringConstants.USER,StringConstants.PASS);
            Statement stat = conn.createStatement();
            ){
            String sqlInsert = " INSERT INTO menu (name, price, qty, tpp, nppt) VALUES ('" + name + "'," + price + "," + qty + "," + tpp + "," + nppt + ")";
